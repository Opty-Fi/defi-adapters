...
export function shouldStakeLikeBeefyFinanceAdapter(token: string, pool: PoolItem): void {
  it(`should stake mooPolygon${token} ,claim rewards, harvest those rewards, and then unstake and withdraw mooPolygon${token} from mooPolygon${token} staking pool of Beefy Finance`, async function () {
    // beefy finance's deposit vault instance
    const beefyDepositInstance = await hre.ethers.getContractAt("IBeefyDeposit", pool.pool);
    // beefy lpToken decimals
    const decimals = await beefyDepositInstance.decimals();
